...
export async function loadState(codexSessionId) {
  try {
    ...
    if (error.code !== "ENOENT") throw new Error("CAPTURE_STATE_REQUIRES_RECONCILIATION");
...
export async function saveState(state, { touch = true } = {}) {
  ...
  const directory = await open(getStateDir(), "r");
...
export async function withSessionLock(codexSessionId, fn, { waitMs = 0, staleMs = 300_000 } = {}) {
  ...
  while (true) {
    try {
      ...
      if (ageMs > staleMs) {
        throw new Error("CAPTURE_LOCK_REQUIRES_RECONCILIATION");
      }
